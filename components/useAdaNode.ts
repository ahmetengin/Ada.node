...
export const useAdaNode = () => {
  ...
  const simulateMeshCommunication = useCallback(async (
    ...
    if (isVotingEnabled) {
        addLog(LogType.VOTING, `Running majority vote for task: ${requestPrompt.substring(0, 100)}...`, fromNode.name);
        ...
        const voteResult = await performMajorityVote(votingPrompt);
  ...
  const executeTask = useCallback(async (task: TaskDetails, isVotingEnabled: boolean) => {
    ...
    await sleep(200);
    addLog(LogType.INFO, `Bağlantı FastRTC mesh protokolü üzerinden kuruluyor... (MAKER Mode: ${isVotingEnabled ? 'ON' : 'OFF'})`, 'Ada Koordinatör');
    ...
    try {
      ...
      const commonCommParams = (req: string, res: string, ack: string) => [req, res, ack, isVotingEnabled] as const;
